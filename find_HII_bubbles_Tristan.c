...
/*
  USAGE: find_HII_bubbles [-p <num of processors>] <redshift> [<ionization efficiency factor zeta>] [<Tvir_min> <ionizing mfp in ionised IGM> <Alpha, power law for ionization efficiency>]
...
int main(int argc, char ** argv){
    ...
    FILE *F, *pPipe;
    FILE *F2; //For outputting original Fcoll
    FILE *F_input; //Fcoll input file
    float *Fcoll_input;
    ...
    ALPHA =  EFF_FACTOR_PL_INDEX; // T =0 for us
    ...
    if (argc == (arg_offset+2)){
        ...
        fprintf(stderr, "USAGE: find_HII_bubbles <redshift> [<ionization efficiency factor zeta>] [<Tvir_min> <ionizing mfp in ionised IGM> <Alpha, power law for ionization efficiency>]\nAborting...\n");
        return -1;
    }
    ...
    //----------Importing our Fcoll----------
    Fcoll_input = (float *) malloc(sizeof(float)*HII_TOT_FFT_NUM_PIXELS); //array for our input of Fcoll
    if (!Fcoll_input){
        fprintf(stderr, "find_HII_bubbles: Error allocating memory for Fcoll box\nAborting...\n");
        fprintf(LOG, "find_HII_bubbles: Error allocating memory for Fcoll box\nAborting...\n");
        fclose(LOG); fftwf_cleanup_threads(); return -1;
    }
    sprintf(filename, "../Boxes/Fcoll_input_file");
    F_input = fopen(filename, "rb"); //File
    if (!F_input){
        fprintf(stderr, "find_HII_bubbles: Unable to open file: %s\n", filename);
        fprintf(LOG, "find_HII_bubbles: Unable to open file: %s\n", filename);
        free(Fcoll_input);
    ...
    fprintf(stderr, "Reading in Fcoll box\n");
    fprintf(LOG, "Reading in Fcoll box\n");
    if (mod_fread(Fcoll_input, sizeof(float)*HII_TOT_FFT_NUM_PIXELS, 1, F_input)!=1){
        fprintf(stderr, "find_HII_bubbles: Read error occured while reading Fcoll box.\n");
        fprintf(LOG, "find_HII_bubbles: Read error occured while reading Fcoll box.\n");
        fclose(F_input); free(Fcoll_input);
        fclose(LOG); fftwf_cleanup_threads(); return -1;
    }
    fclose(F_input);
    //---------------------------------------
    ...
    while (!LAST_FILTER_STEP){//(R > (cell_length_factor*BOX_LEN/(HII_DIM+0.0))){
        ...
        if (LAST_FILTER_STEP){
            ...
            // renormalise the collapse fraction so that the mean matches ST,
        ...
        //T f_coll above = sum of Fcoll
        //T f_coll below = value of Fcoll at each pixel
        ...
        for (x=0; x<HII_DIM; x++){
            for (y=0; y<HII_DIM; y++){
                for (z=0; z<HII_DIM; z++){
                    ...
                    xHI_from_xrays = 1;
                    // check if ionised!
                    if (f_coll > f_coll_crit){ // ionised ---------------------------------------

                        if (FIND_BUBBLE_ALGORITHM == 2) // centre method
                    ...
                    /* check if this is the last filtering step.
                     if so, assign partial ionizations to those cells which aren't fully ionised */
    ...
    switch(FIND_BUBBLE_ALGORITHM){
        case 2:
	      ...
	      sprintf(filename, "../Boxes/xH_nohalos_z%06.2f_nf%f_eff%.1f_effPLindex%.1f_HIIfilter%i_Mmin%.1e_RHIImax%.0f_%i_%.0fMpc", REDSHIFT, global_xH, ION_EFF_FACTOR, ALPHA, HII_FILTER, M_MIN, MFP, HII_DIM, BOX_LEN); //this one
            break;
        default:
            if (USE_HALO_FIELD)
    ...
    if (!(F = fopen(filename, "wb"))){
        ...
        fflush(LOG);
        if (mod_fwrite(xH, sizeof(float)*HII_TOT_NUM_PIXELS, 1, F)!=1){ //writing here
    ...
    system("mkdir ../Fcoll_output_files");
    sprintf(Fcoll_filename, "../Fcoll_output_files/Fcoll_output_file_z%06.2f", REDSHIFT);
