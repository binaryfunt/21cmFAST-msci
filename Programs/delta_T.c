...
int main(int argc, char ** argv){
  ...
  if (!(F = fopen(filename, "rb"))){
    fprintf(stderr, "delta_T: Error opening deltax box for reading at %s\n", filename);
    fprintf(LOG, "delta_T: Error opening deltax box for reading at %s\n", filename);
    free(xH); free(deltax);
    fclose(LOG); fftwf_cleanup_threads(); return -1;
  }
  ...
  if (USE_TS_IN_21CM){
    ...
    if (!(F = fopen(argv[3+arg_offset], "rb") )){
      fprintf(stderr, "delta_T.c: Error opening Ts file %s to read from\nAborting...\n", argv[3+arg_offset]);
      fprintf(LOG, "delta_T.c: Error opening Ts file %s to read from\nAborting...\n", argv[3+arg_offset]);
